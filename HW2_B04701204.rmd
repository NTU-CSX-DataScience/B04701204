...
for (i in Casual_users_dest[1:10,]){
  map$marker(
    c(Casual_users_dest[i,6], Casual_users_dest[i,7]),
    bindPopup = paste(Casual_users_dest[i,4])
  )}
...
for (i in Casual_users_dest[1:8,]){
  map$marker(
    c(Tourist_Attraction[i,2], Tourist_Attraction[i,3]),
    bindPopup = paste(Tourist_Attraction[i,1])
  )}
